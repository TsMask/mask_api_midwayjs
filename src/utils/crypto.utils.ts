 ...
 * @param text 编码字符串
 * @param secret 密钥加密
 * @param type 编码类型 "sha1" | "sha256", 默认使用"sha1"
 * @returns 编码后字符串
 */
export function crypto_hmac(
  text: string,
  secret: string,
 ...
 */
export function encrypy_hash(text: string, type: HashType = 'md5') {
...
/**
 * 微信数据解密
 * @param sessionKey 授权key
 * @param encryptedData 客户端数据
 * @param iv 客户端向量
 */
